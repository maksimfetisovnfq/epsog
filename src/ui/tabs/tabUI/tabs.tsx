...
export const BasicTab = ({
    ...
    const tabLabelsNode = (
        ...
        >
            <Box>
                <Tabs value={tabValue} onChange={handleChange} sx={tabsSx} aria-label="basic tabs example">
    ...
    return (
        <Box>
            {tabLabelsContainer ? (containerEl ? createPortal(tabLabelsNode, containerEl) : null) : tabLabelsNode}

            {tabComponents?.map((Component, index) => {
                return (
                    <CustomTabPanel key={index} index={index} value={tabValue} style={style}>
                        {Component}
                    </CustomTabPanel>
                )
            })}
