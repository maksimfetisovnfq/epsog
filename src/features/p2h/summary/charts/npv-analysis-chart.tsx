...
export const NPVAnalysisChart = () => {
    const data = useSummaryBeks();

    if (!data) return null;

    const npvChartData = data.aggregated.summary.npv_chart_data;

    const stackedBarDataSource = npvChartData.years.map((year, index) => ({
        key: `year-${year}`,
        name: `Year ${year}`,
        valueA: npvChartData.dcfs[index],
        valueB: npvChartData.npv[index],
    }));
